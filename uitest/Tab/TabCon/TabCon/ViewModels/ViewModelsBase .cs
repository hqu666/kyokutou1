	...
	class ViewModelBase : INotifyPropertyChanged, IDataErrorInfo {
		...
		protected class _property<T> : _propetryBase {
				...
				: base(name, vm)
			{
				...
				vm.Properties.Add(this.PropertyName, this);
			}
		...
		/// ICommand を実装するクラス
		/// </summary>
		protected class _delegateCommand : ICommand {
			#region ■ メンバ変数
			...
			{
				if (this._execute_with_param != null)
					this._execute_with_param(parameter);
				else
					this._execute();
			}
